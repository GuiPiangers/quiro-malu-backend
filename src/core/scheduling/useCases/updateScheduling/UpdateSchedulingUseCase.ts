...
export class UpdateSchedulingUseCase {
  ...
  async execute({ userId, ...data }: SchedulingDTO & { userId: string }) {
    ...
    const updateScheduling = schedules.find(schedulingValue => schedulingValue.id === data.id)?.date
    const updateSchedulingDate = updateScheduling && new DateTime(updateScheduling).value

    if (updateSchedulingDate !== data.date && scheduling.isAvailableDate(schedules)) throw new ApiError('Horário indisponível', 400, 'date')


    await this.SchedulingRepository.update({ userId, id: data.id, ...schedulingDTO });
