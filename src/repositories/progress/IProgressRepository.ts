...
export interface IProgressRepository {
    ...
    list(data: { patientId: string, userId: string, config?: { limit: number, offSet: number } }): Promise<ProgressDTO[]>
    count(data: { patientId: string, userId: string }): Promise<[{ total: number }]>
