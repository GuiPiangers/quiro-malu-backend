...
export async function up(knex: Knex): Promise<void> {
  return knex.schema.createTable(ETableNames.AFTER_SCHEDULE_MESSAGES, (table) => {
    table.string("id", 100).primary().index();
    table.string("userId", 100).index().notNullable();
    table.string("name", 255).notNullable().defaultTo("");
    table.integer("minutesAfterSchedule").unsigned().notNullable();
...
export async function down(knex: Knex): Promise<void> {
  return knex.schema.dropTable(ETableNames.AFTER_SCHEDULE_MESSAGES);
}
